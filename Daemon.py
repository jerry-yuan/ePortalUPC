...
if __name__ == '__main__':
    ...
    parser.add_argument("-u", "--username",
                        default=os.getenv("EPORTAL_USERNAME", None), help=help)
    help = "自动登录时使用的密码,默认值为环境变量中EPORTAL_PASSWORD的值"
    parser.add_argument("-p", "--password",
                        default=os.getenv("EPORTAL_PASSWORD", None), help=help)
    help = "自动登录时使用的运营商,默认值为default,可用EPORTAL_SERVICE覆盖"
    parser.add_argument(
        "-s", "--service", default=os.getenv("EPORTAL_SERVICE", "default"), help=help)
    help = "网络状态检查间隔,默认600秒(10分钟),可用EPORTAL_INTERVAL覆盖"
    parser.add_argument(
        "-i", "--interval", default=int(os.getenv("EPORTAL_INTERVAL", 600)), type=int, help=help)
    help = "失败自动重试次数,默认5次,可用EPORTAL_AUTORETRY覆盖"
    parser.add_argument("-r", "--retry-times",
                        default=int(os.getenv("EPORTAL_AUTORETRY", 5)), type=int, help=help)
    ...
    if args.service not in ePortal.getAvaliableISP():
        logging.error("无法识别的服务提供商[{}],当前仅可用{}".format(
            args.service, ",".join(ePortal.getAvaliableISP())))
        exit(-1)
    if args.interval < 1:
                ...
                try:
                    ...
                    if args.retry_times > 0:
                        if retryTimes > args.retry_times:  # 如果已经重试的次数大于等于设定的重试次数,则不再重试
                            logging.error(
                                "重试失败超过{}次,不再重试.".format(args.retry_times))
                            time.sleep(args.interval)
                            retryTimes = 0
                        else:
                            retryTimes += 1
                            if args.retry_interval > 0:
                                logging.info("等待{}秒进行第{}次重试登录.".format(
                                    args.retry_interval, retryTimes))
